...
Details of the data can be found in the following:
  ...
  Development and Application to Compressor Tip Leakage Flow. ASME Paper
  No. GT2021-59077.
...
figure_format(xtitle='Frequency (Hz)', ytitle='SPOD mode energy', 
              zoom=[10**0, 2*10**2, 10**4, 10**12], legend='best')
...
### 4.2 Mode shape
var_names  = ['p','u','v','T']
var_levels = [10**(-4)*np.arange(-3,3.3,0.3),
              10**(-5)*np.arange(-1,1.1,0.1),
              10**(-5)*np.arange(-1,1.1,0.1),
              10**(-7)*np.arange(-3,3.3,0.3)]
...
for vari in plot_vars:
    ...
    for i in range(len(plot_snapshot)):
        ...
        if save_fig:
            plt.savefig(os.path.join(save_path,'t'+str(ti)+'_'+str(var_name)+'.pdf'),
                        bbox_inches='tight')
            plt.close()
