...
#             ncol = 3 (e.g., x, y, control volume size)
...
def jet_contour(q, qlevels, qname, x, y, colormap=cm.coolwarm):
    ...
    # figure format
    figure_format('x','r', [0,10,0,2],'None')    
...
if save_fig:
    plt.savefig(os.path.join(save_path,'Spectrum.pdf'), bbox_inches='tight')
    plt.close()
...
for i in range(len(plot_snapshot)):
    ...
    fig = plt.figure(figsize=(6,4))
    fig = jet_contour(data[ti,:]-data_mean, np.arange(-0.06,0.066,0.006),
                        r'$p-\bar{p} $ (Pa)', grid[:,0], grid[:,1])
    plt.text(7.5,1.7,'t = %.2f s'%(ti*dt), fontsize=14)
    
    if save_fig:
        plt.savefig(os.path.join(save_path,'t'+str(ti)+'_p.pdf'), bbox_inches='tight')
        plt.close()
...
if save_fig:
    jet_contour_anim(t_start, t_end, t_delta, dt, ani_save_name='ori_p_anim.gif',
                       q=data-data_mean, qlevels=np.arange(-0.06,0.066,0.006),
...
if save_fig:
    ...
    jet_contour_anim(0, ts.shape[0], 1, dt*t_delta,
                       ani_save_name='rec_p_anim.gif',
                       q=data_rec, qlevels=np.arange(-0.06,0.066,0.006),
                       qname=r'$p-\bar{p} $ (Pa)', x=grid[:,0], y=grid[:,1],
                       colormap=cm.coolwarm)
